...
void WavetableEngine::setUnison (int numVoices, float detune, float blend)
{
    unisonVoices = juce::jlimit (1, SynthVoice::MAX_UNISON_VOICES, numVoices);
    unisonDetune = juce::jlimit (0.0f, 1.0f, detune);
    unisonBlend  = juce::jlimit (0.0f, 1.0f, blend);

    for (auto& v : voices)
        v.setUnison (unisonVoices, unisonDetune, unisonBlend);
}
...
void WavetableEngine::handleNoteOn (int note, float velocity)
{
    if (monoMode)
    {
        ...
        if (portamentoCoeff >= 0.999f)
            currentMonoNote = targetMonoNote;

        // Find the existing active mono voice or use voice 0
        ...
        monoVoice->startNote ((int)juce::roundToInt (targetMonoNote),
                               velocity, targetMonoNote, portamentoCoeff);
    }
    else
    {
        // Find a free voice
        ...
        target->startNote (note, velocity, (float)note, 1.0f);
    }
}
