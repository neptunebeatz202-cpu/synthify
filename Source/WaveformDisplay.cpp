...
void WaveformDisplay::setLoopPointFromX (int x)
{
    auto pos = (float) x / (float) juce::jmax (1, getWidth());
    pos = juce::jlimit (0.0f, 1.0f, pos);

    if (draggingEndMarker)
        audioProcessor.setLoopEnd (pos);
    else
        audioProcessor.setWavetablePosition (pos);
}

void WaveformDisplay::mouseDown (const juce::MouseEvent& e)
{
    auto startX = getWidth() * audioProcessor.getWavetablePosition();
    auto endX = getWidth() * audioProcessor.getLoopEnd();
    draggingEndMarker = std::abs ((float)e.x - endX) < std::abs ((float)e.x - startX);
    setLoopPointFromX (e.x);
}

void WaveformDisplay::mouseDrag (const juce::MouseEvent& e) { setLoopPointFromX (e.x); }
...
void WaveformDisplay::paint (juce::Graphics& g)
{
    ...
    auto startX = area.getX() + area.getWidth() * audioProcessor.getWavetablePosition();
    auto endX = area.getX() + area.getWidth() * audioProcessor.getLoopEnd();
    g.setColour (juce::Colours::white.withAlpha (0.16f));
    g.fillRect (juce::Rectangle<float> (startX, area.getY(), juce::jmax (1.0f, endX - startX), area.getHeight()));
    g.setColour (juce::Colours::white);
    g.drawLine (startX, area.getY(), startX, area.getBottom(), 2.0f);
    g.drawLine (endX, area.getY(), endX, area.getBottom(), 2.0f);
