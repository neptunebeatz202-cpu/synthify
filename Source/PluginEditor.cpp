    ...
    : AudioProcessorEditor (&p), audioProcessor (p), waveform (p), adsrView (p)
{
    ...
    configureControlLabel (unisonLabel, "UNISON");
    configureControlLabel (detuneLabel, "DETUNE");
    configureControlLabel (blendLabel, "BLEND");
    ...
    configureControlLabel (lengthLabel, "LENGTH");
    configureControlLabel (shapeLabel, "RESHAPE");
    ...
    for (auto* s : { &attackSlider, &decaySlider, &sustainSlider, &releaseSlider,
                     ...
                     &unisonSlider, &detuneSlider, &blendSlider, &lengthSlider, &shapeSlider })
        configureSlider (*s);

    cutoffSlider.setTextValueSuffix (" Hz");
    lengthSlider.setSliderStyle (juce::Slider::IncDecButtons);
    lengthSlider.setTextBoxStyle (juce::Slider::TextBoxLeft, false, 64, 24);
    unisonSlider.setSliderStyle (juce::Slider::IncDecButtons);
    unisonSlider.setTextBoxStyle (juce::Slider::TextBoxLeft, false, 54, 24);
    ...
    lengthAttachment  = std::make_unique<SliderAttachment> (audioProcessor.apvts, "wtLength", lengthSlider);
    shapeAttachment   = std::make_unique<SliderAttachment> (audioProcessor.apvts, "wtShape", shapeSlider);
    unisonAttachment  = std::make_unique<SliderAttachment> (audioProcessor.apvts, "unisonVoices", unisonSlider);
    detuneAttachment  = std::make_unique<SliderAttachment> (audioProcessor.apvts, "unisonDetune", detuneSlider);
    blendAttachment   = std::make_unique<SliderAttachment> (audioProcessor.apvts, "unisonBlend", blendSlider);
...
void WaveForgeEditor::resized()
{
    ...
    wtSlider.setBounds (buttons.reduced (10, 4));
    ...
    auto synth = lower;
    auto k = synth.getWidth() / 9;
    layoutControl (synth.removeFromLeft (k).reduced (6), unisonSlider, unisonLabel);
    layoutControl (synth.removeFromLeft (k).reduced (6), detuneSlider, detuneLabel);
    layoutControl (synth.removeFromLeft (k).reduced (6), blendSlider, blendLabel);
    layoutControl (synth.removeFromLeft (k).reduced (6), lengthSlider, lengthLabel);
    layoutControl (synth.removeFromLeft (k).reduced (6), shapeSlider, shapeLabel);
