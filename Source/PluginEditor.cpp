    ...
    : AudioProcessorEditor (&p), audioProcessor (p), waveform (p), adsrView (p)
{
    ...
    for (auto* b : { &browseButton, &analyzeButton, &cancelButton })
    {
        b->addListener (this);
        addAndMakeVisible (*b);
    }
    ...
    for (auto* s : { &attackSlider, &decaySlider, &sustainSlider, &releaseSlider,
                     ...
                     &unisonSlider, &detuneSlider, &blendSlider })
        configureSlider (*s);
...
void WaveForgeEditor::resized()
{
    ...
    analyzeButton.setBounds (buttons.removeFromLeft (104).reduced (8, 4));
