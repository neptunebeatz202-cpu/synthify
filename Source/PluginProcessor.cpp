...
WaveForgeProcessor::createParameterLayout()
{
    ...
    params.push_back (std::make_unique<juce::AudioParameterInt> (
        "wtLength", "WT Length", 256, 4096, 2048));
    params.push_back (std::make_unique<juce::AudioParameterFloat> (
        "wtShape", "WT Shape",
      ...
      apvts (*this, nullptr, "WaveForge", createParameterLayout())
{
    ...
    apvts.addParameterListener ("unisonVoices",this);
    apvts.addParameterListener ("unisonDetune",this);
    apvts.addParameterListener ("unisonBlend", this);
    ...
    apvts.addParameterListener ("wtLength",    this);
    apvts.addParameterListener ("wtShape",     this);
}
...
void WaveForgeProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    engine.setUnison (
        (int) apvts.getRawParameterValue ("unisonVoices")->load(),
        apvts.getRawParameterValue ("unisonDetune")->load(),
        apvts.getRawParameterValue ("unisonBlend")->load());
                                       ...
                                       juce::MidiBuffer& midiMessages)
{
    ...
    int   unisonVoices = (int) apvts.getRawParameterValue ("unisonVoices")->load();
    float unisonDetune = apvts.getRawParameterValue ("unisonDetune")->load();
    float unisonBlend  = apvts.getRawParameterValue ("unisonBlend")->load();
    ...
    engine.setUnison       (unisonVoices, unisonDetune, unisonBlend);
...
void WaveForgeProcessor::rebuildWavetableFromPosition (float pos)
{
    ...
    int wtSize             = (int) apvts.getRawParameterValue ("wtLength")->load();
    int totalSamples       = sampleBuffer.getNumSamples();
    if (totalSamples <= 0) return;

    wtSize                 = juce::jlimit (16, totalSamples, wtSize);
    ...
    auto shape = apvts.getRawParameterValue ("wtShape")->load();
    if (std::abs (shape) > 0.0001f)
    {
        for (float& s : cycle)
        {
            if (shape > 0.0f)
            {
                auto drive = 1.0f + shape * 12.0f;
                s = std::tanh (s * drive) / std::tanh (drive);
            }
            else
            {
                auto bend = 1.0f + (-shape * 4.0f);
                auto sign = s < 0.0f ? -1.0f : 1.0f;
                s = sign * std::pow (std::abs (s), bend);
            }
        }
    }
...
void WaveForgeProcessor::parameterChanged (const juce::String& paramID, float newValue)
{
    ...
    else if (paramID == "wtLength" || paramID == "wtShape")
    {
